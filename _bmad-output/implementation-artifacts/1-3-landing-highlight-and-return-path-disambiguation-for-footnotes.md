...
Code review 2026-07-16 (Blind Hunter + Edge Case Hunter + Acceptance Auditor). The Acceptance Auditor returned all 7 ACs MET; the Edge Case Hunter then disproved AC2's premise by executing `marked-footnote@1.4.0`, and the reviewer reproduced it. AC3, AC5 and AC7 hold — AC7's four obligations (measured / in both docs / flagged / reversal costed) are all genuinely discharged.

- [x] [Review][Patch] *(was Decision — resolved by Ben 2026-07-16: **keep the feature, correct the record**)* No reversal. Correct `poc/README.md` and the `deferred-work.md` Story 1.3 entry: drop the false "sighted readers are unaffected" claim, document that a footnote taller than ~90 px scrolls its prose above the viewport top, and add the identical-`aria-label` finding as a further cost of the trade-off. The open item stays open and stays Ben's. **The trade-off is worse than the record says** — new input for that item, not a re-litigation of it. Two additions the record does not cover: (1) **Sighted readers are affected too.** `poc/README.md` states "the definition is fully visible on screen and shaded, so sighted readers are unaffected". Fragment navigation start-aligns the target, and the target is now the arrow at the *end* of the definition — so any footnote taller than ~90 px scrolls its own prose above the viewport top. The reader lands on `↩` and must scroll **up** to read it. `scroll-margin-top:90px` on the `li` cannot help: the `li` is no longer the target. (2) **AT users pay the cost and get none of the benefit.** All arrows of a multi-referenced footnote keep an identical accessible name — verified: `[^bgb]`'s two arrows both read `aria-label="Back to reference bgb"`. The feature's entire premise is telling the arrows apart; for a screen-reader user the only distinguishing signal is a colour. So the documented regression buys sighted-only value. The costed ~15-line reversal keeps the landing highlight and drops only the arrow disambiguation — which finding (2) suggests never reached AT users anyway. [poc/dokufix-poc.html:2120, 444, 2685]
- [x] [Review][Patch] AC2/AC4 break where `marked-footnote` mints duplicate ids: for `[^bgb]` (referenced twice) plus `[^bgb-2]`, the library emits `id="footnote-ref-bgb-2"` **twice** (verified by executing the real library), so `querySelector` in `linkFootnoteReturnPaths` returns the first match for both arrows — `[^bgb-2]`'s marker is repointed at footnote **bgb**'s arrow, `footnote-back-bgb-2` is stamped on two elements, and `[^bgb]`'s second marker is never touched. Clicking `[^bgb-2]` lands on the wrong footnote while its hover preview still shows the right text. Needs an already-claimed guard; the loop assumes a 1:1 `refId → ref` mapping the library does not provide [poc/dokufix-poc.html:2115]
- [x] [Review][Patch] `backId` is minted into the same `footnote-*` namespace the library owns for definitions, with no collision check: a document with both `[^back-x]` and `[^x]` produces `id="footnote-back-x"` on a definition `<li>` and on an arrow. `slugify` already guards its reserved `asset-` namespace — that precedent is not applied here [poc/dokufix-poc.html:2118]
- [x] [Review][Patch] The landing highlight relies solely on `background` and `box-shadow`, both of which forced-colors/high-contrast mode strips — leaving only the arrow's `outline` visible, i.e. exactly the element whose reading position is the documented regression. No `@media (forced-colors: active)` fallback in either stylesheet [poc/dokufix-poc.html:2435-2451, 2683-2685]
- [x] [Review][Patch] The `attachFootnotePreviews()` → `linkFootnoteReturnPaths()` order is correct but enforced only by a comment, and reversing it fails **silently**: every preview would clone the `↩` anchor, pass the non-empty check on the glyph, and render as `↩`. `attachFootnotePreviews` has an explicit idempotency guard; this dependency has no equivalent [poc/dokufix-poc.html:1913-1918]
- [x] [Review][Patch] The definition fades in over 250 ms while the arrow inside it snaps instantly and jumps `padding:0 4px` with no transition, shoving neighbouring text sideways — two elements highlighting as one gesture, animating inconsistently. Both stylesheets carried the same omission [poc/dokufix-poc.html:444, 2685]
- [x] [Review][Patch] Dead `try`/`catch` around `CSS.escape` in both footnote passes — `CSS.escape` is total over strings and never throws, so the catch is unreachable; it reads as a guard against malformed labels and is none [poc/dokufix-poc.html:2115, 2133]
- [x] [Review][Patch] `epics.md` FR9 still reads "The existing footnote section, its anchors, and its backrefs continue to work unchanged" — the invariant this story deliberately breaks. Story 1.2's AC5 supersession was recorded; the underlying requirement it derives from was not, though `epics.md` was in this story's File List [_bmad-output/planning-artifacts/epics.md:45]
- [x] [Review][Patch] Story 1.3 has no row in the FR Coverage Map and NFR4 still reads "applies to: 1.1, 1.2", although this story's AC5 discharges NFR4 and its `:has()` use is governed by NFR7. Satisfied on the merits — traceability debt, not risk [_bmad-output/planning-artifacts/epics.md:76-91]
- [x] [Review][Patch] Story 1.2 sits at `review` still asserting "AC5 verified against the real risk: ids/backrefs/hrefs **unchanged**", now factually false, with no pointer forward to 1.3. The claimed in-place supersession comment lives in the uncommitted scratchpad harness [1-2-hover-and-focus-previews-for-footnotes.md:209]

**Dismissed as noise:** (1) preview clones inside `.footnotes` picked up as real arrows (Blind Hunter, `speculative`) — disproved: the clone strips backrefs at `poc/dokufix-poc.html:2077`. (2) `li:has()` double-highlighting list-terminated definitions (Blind Hunter, `speculative`) — disproved by execution: the library appends `↩` at `<li>` level after `</ul>`, not inside the nested `<li>`. (3) `scroll-margin-top:90px` in `READONLY_CSS` clearing a header the exports do not have — harmless, reads as breathing room. (4) Task 3's assertions unauditable because the harness is uncommitted — self-labelled a scope note, not a defect; consistent with the repo's no-test-framework posture and covered by the recorded hands-on pass. (5) Copied URLs carrying `#footnote-back-<id>` — already recorded and accepted in `deferred-work.md`.
